...
public:
	...
	bool m_bRollOnceForAllBulletsInAShot; // 0x668	
private:
...
public:
	...
	float m_flMaxBulletsToProcInShot; // 0x66c	
	...
	bool m_bCanProcMultipleTimesFromSameShot; // 0x670	
	// MPropertyDescription "When true, will only call OnProc if the target hit passes the unit filter."
	bool m_bRequiresTargetFilter; // 0x671	
	// MPropertyDescription "When false, Proc'd bullets cannot be evaded"
	bool m_bCanBeEvaded; // 0x672	
private:
	[[maybe_unused]] uint8_t __pad0673[0x5]; // 0x673
public:
	// MPropertyGroupName "Visuals"
	CResourceNameTyped< CWeakHandle< InfoForResourceTypeIParticleSystemDefinition > > m_TracerAdditionParticle; // 0x678	
	// MPropertyStartGroup "Sounds"
	// MPropertyDescription "Sound to play on the shooter when a bullet rolls a proc when fired."
	CSoundEventName m_OnBulletRolledProcSound; // 0x758	
};
