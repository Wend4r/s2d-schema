...
public:
	...
	GameTime_t m_flLastPickupCheck; // 0xa64	
	bool m_bPlayerCounterListenerAdded; // 0xa68	
	bool m_bPlayerInTriggerRadius; // 0xa69	
private:
	[[maybe_unused]] uint8_t __pad0a6a[0x6]; // 0xa6a
public:
	CStrongHandle< InfoForResourceTypeIParticleSystemDefinition > m_hSpawnParticleEffect; // 0xa70	
...
public:
	...
	CStrongHandle< InfoForResourceTypeIParticleSystemDefinition > m_hPickupParticleEffect; // 0xa90	
	CUtlSymbolLarge m_pPickupSoundEffect; // 0xa98	
	CUtlSymbolLarge m_pPickupScriptFunction; // 0xaa0	
	CStrongHandle< InfoForResourceTypeIParticleSystemDefinition > m_hTimeoutParticleEffect; // 0xaa8	
	...
	CHandle< CBaseFilter > m_hPickupFilter; // 0xac8	
private:
	[[maybe_unused]] uint8_t __pad0acc[0x4]; // 0xacc
public:
	CEntityIOOutput m_OnPickup; // 0xad0	
	CEntityIOOutput m_OnTimeout; // 0xaf8	
	CEntityIOOutput m_OnTriggerStartTouch; // 0xb20	
	CEntityIOOutput m_OnTriggerTouch; // 0xb48	
	CEntityIOOutput m_OnTriggerEndTouch; // 0xb70	
...
public:
	CHandle< CItemGenericTriggerHelper > m_hTriggerHelper; // 0xbb8	
