...
public:
	...
	CPropDataComponent m_CPropDataComponent; // 0x770	
	Materials m_Material; // 0x7b0	
	CHandle< CBaseEntity > m_hBreaker; // 0x7b4	
	Explosions m_Explosion; // 0x7b8	
private:
...
public:
	...
	EOverrideBlockLOS_t m_nOverrideBlockLOS; // 0x7dc	
	CEntityIOOutput m_OnBreak; // 0x7e0	
	CEntityOutputTemplate< float32 > m_OnHealthChanged; // 0x808	
	PerformanceMode_t m_PerformanceMode; // 0x830	
	CHandle< CBasePlayerPawn > m_hPhysicsAttacker; // 0x834	
	GameTime_t m_flLastPhysicsInfluenceTime; // 0x838	
